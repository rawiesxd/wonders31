...
if(!muteRole) return;
  ...
  if (MessageData[msg.author.id].MesssageNumber === 15) {
    ...
    if (msg.deletable) msg.delete();// yap koy bunu ok
      const spambed = new Discord.MessageEmbed()
        .setColor("GOLD")
      .setAuthor("Wonders", client.user.avatarURL())//sa as kanka çok az komut kaldı ha v12 yapılmamış .D 9 tane falan bide müzik sistemi eklicez o kadar
 ...
 return msg.channel.send(atıldı).then(atıldı => atıldı.delete({time:5000}))
    ...
    msg.member.ban({reason: `Wonders Anti Spam Sistemi`})
