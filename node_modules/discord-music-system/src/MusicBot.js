...
class MusicBot {
  ...
  constructor(client, options = {}) {
     ...
     */
    this.errorMsg =
      "> \x1b[33m[Discord Music System], \x1b[31mError: \x1b[37m ";
     ...
     */
    this.interErrorMsg =
      "> \x1b[33m[Discord Music System], \x1b[31mIntern Error: \x1b[37m ";
     ...
     */
    this.warnMsg =
      "> \x1b[33m[Discord Music System], \x1b[31mWarning \x1b[37m ";
  ...
  async onMessage(message) {
    if (!message) {
      throw new Error(
        this.errorMsg + "The message is required on the onMessage function."
      );
    }
    if (message.channel.type === "dm") {
      return this.sendErrorEmbed(
        this.language.messages.noPrivateMessages,
        message
      );
    }
    ...
    // Play command
    if (
      message.content.startsWith(this.prefix + "play") ||
      message.content.startsWith(this.prefix + "add") ||
      message.content.startsWith(this.prefix + "join")
    ) {
      if (!message.member.voice.channel) {
        return this.sendErrorEmbed(
          this.language.messages.voiceChannelNeeded,
          message
        );
      }
      ...
      if (!permissions.has("CONNECT") || !permissions.has("SPEAK")) {
        return this.sendErrorEmbed(
          this.language.messages.cannotConnect,
          message
        );
      }
      ...
      if (!serverQueue) {
        message.channel.send(
          new MessageEmbed()
            .setColor("YELLOW")
            .setTimestamp()
            .setAuthor(this.language.embeds.createdMusicPlayer.title)
            .setDescription(this.language.embeds.createdMusicPlayer.description)
        );
      }

      if (serverQueue && !this.sameVoiceChannel(message.member)) {
        return this.sendErrorEmbed(
          this.language.messages.sameVoiceChannel,
          message
        );
      }
    ...
    // Stop command
      ...
      message.content === this.prefix + "stop" ||
      message.content === this.prefix + "kill" ||
      message.content === this.prefix + "destroy" ||
      message.content === this.prefix + "leave"
    ) {
      if (!message.member.voice.channel) {
        return this.sendErrorEmbed(
          this.language.messages.voiceChannelNeeded,
      ...
      if (!serverQueue) {
        return this.sendErrorEmbed(
          this.language.messages.nothingPlaying,
          message
        );
      }
      ...
      serverQueue.textChannel.messages
        .fetch({
          limit: 1,
          around: this.messagesReactions.get(serverQueue.textChannel.guild.id),
        })
        .then(async (messages) => {
    ...
    // Now playing command
      ...
      message.content === this.prefix + "np" ||
      message.content === this.prefix + "nowplaying" ||
      message.content === this.prefix + "current"
    ) {
      const serverQueue = this.queue.get(message.guild.id);
      if (!serverQueue) {
        return this.sendErrorEmbed(
      ...
      let nowPlaying = new MessageEmbed()
        ...
        .addField(
          ...
          "`▶ " +
            ...
            "] " +
            (song.duration == 0
              ? ` ${this.language.embeds.nowPlaying.live}`
      ...
      if (song.duration > 0) {
        nowPlaying.setFooter(
          `${this.language.embeds.nowPlaying.remainingTime} ` +
    ...
    // Skip command
      ...
      message.content === this.prefix + "skip" ||
      message.content === this.prefix + "next" ||
      ...
      serverQueue.textChannel.messages
        .fetch({
          limit: 1,
          around: this.messagesReactions.get(message.guild.id),
        })
        .then(async (messages) => {
          await messages.first().reactions.removeAll().catch(console.error);
      ...
      return message.channel.send(
        new MessageEmbed()
          .setColor("ORANGE")
          .setFooter(
            `${this.language.embeds.skipSong.askedBy} ` + message.author.tag,
            message.author.displayAvatarURL()
          )
          .setDescription(
            "⏩ `" +
              serverQueue.songs[0].title +
              `\` ${this.language.embeds.skipSong.beenSkipped}`
          )
      );
    }
    // Queue command
    else if (
      message.content === this.prefix + "queue" ||
      message.content === this.prefix + "list" ||
      message.content === this.prefix + "show"
    ) {
      if (!message.member.voice.channel) {
        return this.sendErrorEmbed(
          this.language.messages.voiceChannelNeeded,
    ...
    // Volume command
      ...
      message.content.startsWith(this.prefix + "volume") ||
      message.content.startsWith(this.prefix + "setvolume")
    ) {
      if (!message.member.voice.channel) {
        return this.sendErrorEmbed(
          this.language.messages.voiceChannelNeeded,
      ...
      if (!isNaN(volumeNumber) && volumeNumber >= 0 && volumeNumber <= 100) {
        message.channel.send(
          new MessageEmbed()
            .setColor("GOLD")
            .setTitle(this.language.embeds.volumeEmbed.title)
            .setDescription(
              `${this.language.embeds.volumeEmbed.changedFrom} \`${
                serverQueue.volume * 100
              }\` ${this.language.embeds.volumeEmbed.to} \`${volumeNumber}\``
            )
        );
    ...
    // Pause command
      ...
      if (serverQueue && serverQueue.playing) {
        ...
        return message.channel.send(
          new MessageEmbed()
            .setTitle(this.language.embeds.pauseEmbed.title)
            .setColor("GOLD")
            .setDescription(this.language.embeds.pauseEmbed.description)
            .setTimestamp()
            .setFooter(
              `${this.language.embeds.pauseEmbed.askedBy} ` +
                message.author.tag,
              message.author.displayAvatarURL()
            )
        );
    ...
    // Resume command
      ...
      if (serverQueue && !serverQueue.playing) {
        ...
        return message.channel.send(
          new MessageEmbed()
            .setTitle(this.language.embeds.resumeEmbed.title)
            .setColor("GOLD")
            .setDescription(this.language.embeds.resumeEmbed.description)
            .setTimestamp()
            .setFooter(
              `${this.language.embeds.resumeEmbed.askedBy} ` +
                message.author.tag,
              message.author.displayAvatarURL()
            )
        );
    ...
    // Remove command
    if (
      message.content.startsWith(this.prefix + "remove") ||
      message.content.startsWith(this.prefix + "delete")
    ) {
      if (!message.member.voice.channel) {
        return this.sendErrorEmbed(
          this.language.messages.voiceChannelNeeded,
      ...
      if (!isNaN(index) && index >= 1 && serverQueue.songs.length > index) {
        message.channel.send(
          new MessageEmbed()
            .setTitle(this.language.embeds.removeEmbed.title)
            .setColor("AQUA")
            .setDescription(
              `[${serverQueue.songs[index].author}](${serverQueue.songs[index].authorUrl})\n**[${serverQueue.songs[index].title}](${serverQueue.songs[index].url})**\n\`${this.language.embeds.removeEmbed.removedBy} ${message.author.username}.\``
            )
            .addField(
              this.language.embeds.removeEmbed.published,
              "`" + serverQueue.songs[index].published + "`",
              true
            )
            .addField(
              this.language.embeds.removeEmbed.views,
              "`" + serverQueue.songs[index].views + "`",
              true
            )
            .setFooter(
        ...
        this.sendErrorEmbed(
          this.language.messages.validNumberPosition,
          message
        );
      }
    }
  ...
  isYouTubeVideoURL(
    url,
    videoClass = /^(https?:\/\/)?(www\.)?(m\.)?(youtube\.com|youtu\.?be)\/.+$/gi
  ) {
    if (!url) {
      throw new Error(
        this.interErrorMsg +
          "The URL is required on the isYouTubeVideoURL function."
      );
    }
  ...
  isYouTubePlaylistURL(url, playlistClass = /^.*(list=)([^#\&\?]*).*/gi) {
    if (!url) {
      throw new Error(
        this.interErrorMsg +
          "The URL is required on the isYouTubePlaylistURL function."
      );
    }
  ...
  sendErrorEmbed(errorMessage, message) {
    if (!errorMessage) {
      throw new Error(
        this.interErrorMsg +
          "The error message is required on the sendErrorEmbed function."
      );
    }
    ...
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the sendErrorEmbed function."
      );
    }
    ...
    return message.channel.send(
      new MessageEmbed()
        ...
        .setAuthor(this.language.embeds.errorEmbed.title)
    );
  }
  ...
  async playQuery(args, message) {
    if (!args) {
      throw new Error(
        this.interErrorMsg + "The args are required on the playQuery function."
      );
    }
    ...
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the playQuery function."
      );
    }
    ...
    if (infos.results.length === 0) {
      return this.sendErrorEmbed(
        this.language.messages.couldNotBeFound,
        message
      );
    }
    ...
    if (!songInfo) {
      return this.sendErrorEmbed(
        this.language.messages.restrictedOrNotFound,
        message
      );
    }
    ...
    const song = {
      ...
      thumbnailUrl:
        songInfo.player_response.videoDetails.thumbnail.thumbnails.pop().url,
    ...
    if (serverQueue) {
      ...
      return await message.channel.send(
        new MessageEmbed()
          ...
          .setThumbnail(song.thumbnailUrl)
          .setDescription(
            `**\`${song.title}\`** ${this.language.messages.hasBeenAdded}`
          )
      );
    }
    ...
    try {
      ...
      return this.sendErrorEmbed(
        `${this.language.messages.cannotConnect} \`${error}\``,
        message
      );
    }
  }
  ...
  async playSong(song, message, serverQueue) {
    if (!message) {
      throw new Error(
        this.interErrorMsg + "The message is required on the playSong function."
      );
    }
    ...
    if (!song) {
      ...
      return await message.channel.send(
        new MessageEmbed()
          .setColor("YELLOW")
          .setTimestamp()
          .setAuthor(this.language.embeds.playerDestroyedEmbed.author)
          .setDescription(this.language.embeds.playerDestroyedEmbed.description)
      );
    }
    ...
    if (!serverQueue) {
      ...
      var playingMessage = await queue.textChannel.send(
        new MessageEmbed()
          .setColor("RED")
          .setDescription(
            `[${song.author}](${song.authorUrl})\n**[${song.title}](${song.url})**\n\`${this.language.embeds.playEmbed.addedBy} ${message.author.username} ${this.language.embeds.playEmbed.luckySearch}\``
          )
          .addField(
            this.language.embeds.playEmbed.published,
            "`" + song.published + "`",
            true
          )
          .addField(
            this.language.embeds.playEmbed.views,
            "`" + song.views + "`",
            true
          )
          .setFooter(
  ...
  async updateClientPresence(message) {
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the updateClientPresence function."
      );
    }

    if (!message) {
      this.client.user.setActivity(this.presence.nothing, {
        type: "LISTENING",
      });
    }
    ...
    if (!serverQueue) {
      ...
      this.client.user.setActivity("🎵 " + serverQueue.songs[0].title, {
        type: "LISTENING",
      });
    }
  }
  ...
  async getSongLyrics(args, message) {
    if (!args) {
      throw new Error(
        this.interErrorMsg +
          "The args are required on the getSongLyrics function."
      );
    }
    ...
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the getSongLyrics function."
      );
    }
    ...
    var msg =
      (await message.channel.send(
        new MessageEmbed()
          .setTimestamp()
          .setAuthor(
            this.language.embeds.lyricsEmbed.searching,
            "https://www.aerobusbcn.com/sites/all/themes/aerobus/images/ajax-loader.gif"
          )
          .setColor("YELLOW")
      )) ||
      message.send(
    ...
    var res = await fetch(
      `https://some-random-api.ml/lyrics?title=${encodeURIComponent(args)}`
    );
    ...
    if (lyrics.error) {
      return msg.edit(
        new MessageEmbed()
          ...
          .setTimestamp()
          .setAuthor(this.language.embeds.errorEmbed.title)
          .setDescription("❌ " + this.language.messages.couldNotBeFound)
      );
    }
    ...
    return msg.edit(
      new MessageEmbed()
        ...
        .setTimestamp()
        .setFooter(
          this.language.embeds.lyricsEmbed.askedBy + message.author.tag,
          message.author.displayAvatarURL()
        )
  ...
  async sendQueueEmbed(serverQueue, message) {
    if (!serverQueue) {
      throw new Error(
        this.interErrorMsg +
          "The serverQueue is required on the sendQueueEmbed function."
      );
    }
    ...
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the sendQueueEmbed function."
      );
    }
    ...
    const QueueEmbed = new MessageEmbed()
      ...
      .setTitle(
        "🎹 `" +
          message.guild.name +
          "`" +
          this.language.embeds.queueEmbed.queue
      );
  ...
  sameVoiceChannel(member) {
    if (!member) {
      throw new Error(
        this.interErrorMsg +
          "The member is required on the sameVoiceChannel function."
      );
    }
  ...
  async reactionsMessageSystem(message, queue, song, basicMessage) {
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the reactionMessageSystem function."
      );
    }
    ...
    if (!queue) {
      throw new Error(
        this.interErrorMsg +
          "The queue is required on the reactionMessageSystem function."
      );
    }
    ...
    if (!song) {
      throw new Error(
        this.interErrorMsg +
          "The song is required on the reactionMessageSystem function."
      );
    }
    ...
    if (!basicMessage) {
      throw new Error(
        this.interErrorMsg +
          "The basicMessage is required on the reactionMessageSystem function."
      );
    }

    this.messagesReactions.set(message.guild.id, message.id);
    const filter = (reaction, user) =>
      (user.id !== this.client.user.id && reaction.emoji.name === "⏯") ||
      reaction.emoji.name === "⏭" ||
      reaction.emoji.name === "🔈" ||
      reaction.emoji.name === "🔊" ||
      reaction.emoji.name === "⏹";
  ...
  async playVideoUrl(url, message) {
    if (!url) {
      throw new Error(
        this.interErrorMsg + "The url is required on the playVideoUrl function."
      );
    }
    ...
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the playVideoUrl function."
      );
    }
    ...
    if (serverQueue && serverQueue.songs && serverQueue.playing) {
      ...
      if (!songInfo) {
        return this.sendErrorEmbed(
          this.language.messages.errorWhileParsingVideo,
          message
        );
      }
      ...
      return await message.channel.send(
        new MessageEmbed()
          ...
          .setThumbnail(song.thumbnailUrl)
          .setDescription(
            `**\`${song.title}\`** ${this.language.embeds.addEmbed.hasBeenAdded}`
          )
      );
    } else {
      const songInfo = await ytdl.getInfo(url);
  ...
  async playPlaylist(query, message) {
    if (!query) {
      throw new Error(
        this.interErrorMsg +
          "The query is required on the playPlaylist function."
      );
    }
    ...
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the playPlaylist function."
      );
    }
        ...
        if (playlist) {
          ...
          if (serverQueue && serverQueue.playing) {
            message.channel.send(
              new MessageEmbed()
                .setColor("DARK_PURPLE")
                .setTimestamp()
                .setThumbnail(playlist.items[0].thumbnail)
                .setDescription(
                  `${this.language.embeds.addEmbed.playlist} **\`${playlist.title}\`** ${this.language.embeds.addEmbed.hasBeenAdded}`
                )
            );
          }
          playlist.items.map(async (i) => {
            if (serverQueue && serverQueue.playing === true) {
              ...
              if (!songInfo) {
                return this.sendErrorEmbed(
                  this.language.messages.errorWhileParsingPlaylist,
                  message
                );
              }
              const song = {
                ...
                duration: songInfo.videoDetails.lengthSeconds,
                thumbnailUrl:
                  songInfo.player_response.videoDetails.thumbnail.thumbnails.pop()
                    .url,
                published: songInfo.videoDetails.publishDate,
                views: songInfo.player_response.videoDetails.viewCount,
              };
  ...
  async musicMessageReact(message) {
    if (!message) {
      throw new Error(
        this.interErrorMsg +
          "The message is required on the musicMessageReact function."
      );
    }
