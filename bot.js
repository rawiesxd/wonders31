...
client.ayarlar = {
  ...
  "botİsim": "Wonders",
  "embedRenk": "GOLD",
  "embedFooter": "Copyright © Wonders Bot 2022 | Sponsor: BelenHosting",
  "version": "2.1",
...
client.on("ready", async() => {
  ...
  console.log("•--------------•")
  client.user.setActivity(`❤️ Marcellus! 🔎 Yeni Güncellemeler! | ${client.ayarlar.prefix}sponsor`)
...
client.on("message", async message => {
      ...
      //  const command = args.shift().toLowerCase()
        ...
        if (cmd) {
      if(client.ws.ping > 2000) return message.channel.send(new Discord.MessageEmbed().setDescription(`• Değerli kullanıcımız sizin sunucunuzunda yüksek değerde botun gecikmesi olduğunu farkettik. Bu gecikme sorununu düzeltmek için sunucunun bölgesini değiştirmeniz, birkaç dakika beklemeniz ya da [destek sunucusunda](https://discord.gg/CTsvk5XuUk) bu soruyu sorarak ek çözüm öğrenebilirsiniz.
      :satellite: Sunucunuzda ölçülen yüksek gecikme değeri: **${client.ws.ping}ms**`).setTitle("Yüksek Gecikme Oranı!").setThumbnail(client.ayarlar.resim).setColor("GOLD"));
if(db.fetch(`bakimmod`)) {
  if(!client.ayarlar.sahip.includes(message.author.id)) return message.reply(new Discord.MessageEmbed().setDescription(`**• Verdiğimiz geçici rahatsızlıktan dolayı özür dileriz. En kısa sürede yeniden hizmetinize açılacaktır.**`).setTitle("<:wunlem:911958090298032228> **Wonders geçici bir süreliğine bakımdadır.**").setThumbnail(client.ayarlar.resim).setColor("GOLD"));
}  
    ...
    //  .setImage("https://cdn.discordapp.com/attachments/742686392416731136/742688052496826428/20200811_131630.png")
      ...
      return message.channel.send(karalisteee)
           ...
           .setColor(client.ayarlar.embedRenk)
           .setAuthor("d-dostum biraz yavaşla!", message.author.avatarURL({dynamic: true}))
        ...
        let time = Date.now() - message.author.createdTimestamp
          ...
          if (time < 604800000) {
            ...
            /* let fakehesapkoruma = new Discord.MessageEmbed()
             .setColor(client.ayarlar.embedRenk)
             .setDescription(`Dostum, Sahibim tarafından alınan önlem sonucunda **Yeni Açılmış** hesapların Wonders'i kullanması yasaklanmıştır, bu nedenle ${text.join(", ")} süresi boyunca beklemelisin!`)
             .setFooter("Copyright © Wonders Bot 2022", client.user.avatarURL())
            return message.channel.send(fakehesapkoruma) */
            } 
			
			if(!message.author.avatarURL()) return message.channel.send("Wonders'i kullanmak istiyorsan eğer bir avatarın olmalı!")
      ...
      db.add(`komutkullanım31`, 1)  
      db.add(`komutkullanım31_kişiyeözel_${message.author.id}`, 1)  

    let qomtkanal = client.channels.cache.get("904306238517555200")
    let komutkullanan = db.get(`komutkullanım31`)
    const qomttt = new Discord.MessageEmbed()
  .setDescription(`${message.author.username} (${message.author.id}) Adlı Kullanıcı **${command}** adlı komutu kullandı.
...
client.on("message", async(message) => {
  ...
  if(message.content === `<@${client.user.id}>` || message.content === `<@!${client.user.id}>`) {
    ...
    .setColor(client.ayarlar.embedRenk)
    .setDescription(`
    • | Selam ${message.author}, Ben **${client.ayarlar.botİsim} Bot** Sanırsam beni etiketlediğini gördüm!
    • | Önemli Komutlar: \`${prefix}yardım, ${prefix}sponsor, ${prefix}davet\`
    `)
...
member.roles.add(rol);
  if (db.has(`üyelikk_${member.id}`) === true) {
    ...
    member.guild.channels.cache.get(kanal).send(`<a:goldkrsm:857214678975250472> ${member} Adlı Gold Üye Sunucuya Katıldı! Seninle beraber **${member.guild.memberCount}** Kişiyiz! <:neutralyes:910935020120735806>`).catch(s => console.log(s))
  } else {
  if (!mesaj) {
    client.channels.cache.get(kanal).send(`<a:girisypti:857216042568908820> Hoşgeldin ${member} Otomatik Rol Verildi Seninle **${member.guild.memberCount}** kişiyiz <:neutralyes:910935020120735806>`).catch(s => console.log(s))
    return;
  } else {
    var mesaj2 = mesaj
...
client.on("guildMemberAdd", async member => {
  ...
  if(!member.guild.roles.cache.get(rol)){
    ...
    if(member.guild.channels.cache.get(kanal)){
     member.guild.channels.cache.get(kanal).send(`Belirlenen Rol Bulunamadı! Bu Yüzden Sıfırlandı! <:neutralyes:910935020120735806>`)
...
member.roles.add(rol);
  if (!mesaj) {
    client.channels.cache.get(kanal).send(`<a:girisypti:857216042568908820> Hoşgeldin ${member} Otomatik Rol Verildi Seninle **${member.guild.memberCount}** kişiyiz <:neutralyes:910935020120735806> [Bot]`).catch(s => console.log(s))
    return;
  } else {
    var mesaj2 = mesaj
      .replace(`{sunucuadı}`, `**${member.guild.name}**`)
...
client.on("guildMemberAdd", async member => {
  ...
  if (!member.guild.channels.cache.get(kanal)) return;
   ...
   return client.channels.cache.get(kanal).send(`<:neutralyes:910935020120735806> Sayaç Sıfırlandı! \`${member.guild.memberCount}\` Kişiyiz!`).catch(s => console.log(s))
  }
  if (db.has(`üyelikk_${member.id}`) === true) {
    member.guild.channels.cache.get(kanal).send(`<a:goldkrsm:857214678975250472> ${member} Adlı Premium Üye Sunucuya Katıldı! **${sayaç}** Kişi Olmamıza **${sonuç}** Kişi Kaldı **${member.guild.memberCount}** Kişiyiz! <:neutralyes:910935020120735806>`).catch(s => console.log(s))
  } else { 
    if (!mesaj) {
    return client.channels.cache.get(kanal).send(`<a:girisypti:857216042568908820> ${member} Adlı Kullanıcı Sunucuya Katıldı! **${sayaç}** Kişi Olmamıza **${sonuç}** Kişi Kaldı **${member.guild.memberCount}** Kişiyiz! <:neutralyes:910935020120735806>`).catch(s => console.log(s))
  } else {
...
client.on("guildMemberRemove", async member => {
  ...
  if (!sayaç) return;
    if (member.guild.channels.cache.get(kanal)){
  if (db.has(`üyelikk_${member.id}`) === true) {
    member.guild.channels.cache.get(kanal).send(`<a:goldkrsm:857214678975250472> ${member} Adlı Gold Üye Sunucudan Ayrıldı. **${sayaç}** Kişi Olmamıza **${sonuç}** Kişi Kaldı **${member.guild.memberCount}** Kişiyiz!`).catch(s => console.log(s))
  } 
  if (!mesaj) {
    return client.channels.cache.get(kanal).send(`<a:cikisypti:857216066769387521> ${member} Adlı Kullanıcı Sunucudan Ayrıldı. **${sayaç}** Kişi Olmamıza **${sonuç}** Kişi Kaldı **${member.guild.memberCount}** Kişiyiz!`).catch(s => console.log(s))
  }
  if (mesaj) {
    const mesaj31 = mesaj.replace("{kullanıcı}", `${member}`).replace("{sunucuadı}", `**${member.guild.name}**`).replace("{sunucuüyesayısı}", `**${member.guild.memberCount}**`).replace("{kullanıcı_adı}", `**${member.user.username}**`).replace("{kalanüye}", `**${sonuç}**`)
    return client.channels.cache.get(kanal).send(mesaj31).catch(s => console.log(s))
  ...
  if(capslock === "aktif") {
    ...
    let caps = message.content.replace(" ", "").toUpperCase();
      ...
      if(message.content === caps) {
        let mesajj = [
          `${message.author}, **Dostum bu sunucuda caps açamazsın!**`,
        ]
  ...
  if(capslock === "aktif") {
    if(!oldMessage.member.hasPermission("MANAGE_MESSAGES")) {
      ...
      if(newMessage.content.toUpperCase()) {
        let mesajj = [
          `${oldMessage.author}, **Dostum mesajını düzenleyerek caps açamazsın!**`,
        ]
          let mesaj = Math.floor((Math.random() * mesajj.length));
...
client.on("message", message => {
  ...
  if(küfür === "aktif") {
   ...
   let mesajj = [
    `${message.author}, **Dostum küfür etmemelisin!**`,
  ]
    let mesaj = Math.floor((Math.random() * mesajj.length));
...
client.on("messageUpdate", (oldMessage, newMessage) => {
  ...
  if(küfür === "aktif") {
   ...
   let kufur = require("./küfürler.json")
   let mesajj = [
    `${oldMessage.author}, **Mesajını düzenleyerek küfür edemezsin!**`,
  ]
    let mesaj = Math.floor((Math.random() * mesajj.length));
...
client.on("message", message => {
  ...
  if(linkk === "aktif") {
   ...
   let mesajj = [
    `${message.author}, **Dostum link atmamalısın!**`,
  ]
    let mesaj = Math.floor((Math.random() * mesajj.length));
...
client.on("messageUpdate", (oldMessage, newMessage) => {
  ...
  if(linkk === "aktif") {
   ...
   let mesajj = [
    `${oldMessage.author}, **Mesajını düzenleyerek link atamazsın!**`,
  ]
    let mesaj = Math.floor((Math.random() * mesajj.length));
      ...
      if (swearWords.some(word => message.content.includes(word))) {
          try {
            ...
            db.add(`günlük_reklam`, +1)
                let mesajj = [
                  `${message.author}, **Dostum reklam yapmamalısın!**`,
                ]
                let mesaj = Math.floor((Math.random() * mesajj.length));
                message.delete();
            const embed1 = new Discord.MessageEmbed()
  ...
  if (veri === "aktif") {
      ...
      if (swearWords.some(word => newMessage.content.includes(word))) {
          try {
            ...
            db.add(`günlük_reklam`, +1)
                let mesajj = [
                  `${oldMessage.author}, **Mesajını düzenleyerek reklam yapamazsın!**`,
                ]
                let mesaj = Math.floor((Math.random() * mesajj.length));
                newMessage.delete();
            const embed1 = new Discord.MessageEmbed()
...
client.on("guildMemberAdd", async(member) => {
  ...
  .setColor(embedRenk)
  .setDescription(`
  <a:girisypti:857216042568908820> Sunucumuza Hoş Geldin, ${member} !
  <:wkullanci:911949930053140490> Senin sayende **${guild.memberCount}** Üye olduk! 
  <a:hypes:857179846836748327> Hesabın **${tarih}** Tarihinde kurulmuş!
  <a:hedikkat:771840237760544768> Bu Kullanıcı **${kontrol}**!
 <a:hype2:799926767384264725> <@&${yetkili}> Rolündeki yetkililer senin ile ilgilenecektir!
  `)
  .setImage(embedİmage)
...
a.edit(`Butona tıkladığın için <@&${veri.rol}> Rolünü senden aldım.`)
member.roles.remove(veri.rol)

} else {

a.edit(`Butona tıkladığın için <@&${veri.rol}> Rolünü sana verdim.`)
...
member.send(`**${newGuild.name}** sunucusunun **Vanity URL**'sini değiştirdiğin için tüm yetkilerin alındı.`);
guild.owner.user.send(`**${newGuild.name}** sunucunda ki ${member}, sunucunun **Vanity URL**'sini değiştirmeye kalkıştı. Hallettim.\n\n**•** ${oldGuild.vanityURLCode} -> ${newGuild.vanityURLCode}`);
};
});
