...
exports.run = async (client, message) => {
    ...
    if(!db.has(`üyelikk_${message.author.id}`) === "aktif") {
      dbl.hasVoted(message.author.id).then(voted => {
        if(voted === true) {
          if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("<:neutralno:910935142707646484> Bu komutu kullanabilmek için `YÖNETİCİ` iznine ihtiyacın var!")
        setTimeout(() => {
      message.channel.clone({ 
              ...
              .setDescription(`<:neutralyes:910935020120735806> Kanal başarılı bir şekilde kopyalandı ve eski kanal silindi!`)
