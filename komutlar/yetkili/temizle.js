...
exports.run = (client, message, args) => {
 ...
 message.channel.send(`<a:onaylandi:698113364026720267> Başarılı Bir Şekilde **${miktar}** Mesaj Silindi.`).then(msg => {msg.delete({ timeout: 5000 })}).catch(err => { 
   ...
   if(!sayı || isNaN(sayı)) {
	...
	.setAuthor(`Wonders Bot | Sil Komutu`, message.author.avatarURL({ dynamic: true }))
	.setDescription(`<:neutralno:910935142707646484> Eğer mesajları silmemi istiyorsan lütfen silinicek mesaj miktarını belirt!`)
   ...
   if(sayı > 100) {
	...
	.setColor(client.ayarlar.embedRenk)
	.setAuthor(`Wonders Bot | Sil Komutu`, message.author.avatarURL({ dynamic: true }))
	.setDescription(`<:neutralno:910935142707646484> Üzgünüm ancak Discord kurallarına göre 100'den fazla mesaj silemem!`)
	.setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
	return message.channel.send(embed) 
   }
   ...
   const embed = new Discord.MessageEmbed()
	.setColor(client.ayarlar.embedRenk)
	.setAuthor(`Wonders Bot | Sil Komutu`, message.author.avatarURL({ dynamic: true }))
	.setDescription(`<:neutralyes:910935020120735806> Başarılı bir şekilde **${sayı}** Adet mesaj sildim!`)
	.setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
