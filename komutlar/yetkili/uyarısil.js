...
exports.run = async (client, message, args) => {
   ...
   if(!db.has(`uyarı_${member.id}_${message.guild.id}`) === true) {
       var yok = new Discord.MessageEmbed()
           ...
           .setDescription(`<:neutralno:910935142707646484> ${member} Adlı kişinin hiç bir uyarısı yok!`)
   ...
   if(data.length === 1) {
    ...
    var yok = new Discord.MessageEmbed()
           .setAuthor(`${client.ayarlar.botİsim} Bot`, message.author.avatarURL({dynamic:true}))
           .setColor(client.ayarlar.embedRenk)
           .setDescription(`<:neutralyes:910935020120735806> Başarılı bir şekilde ${member} adlı kişinin, **${id}** İD'li uyarısını sildim!`)
           .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({dynamic:true}))
           return message.channel.send(yok);
   } else {
