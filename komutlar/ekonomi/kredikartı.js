...
exports.run = async(client, message, args, prefix) => {
    ...
    if(args[0] === "oluştur") {
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralno:910935142707646484> Başarılı bir şekilde kredi kartınız oluşturuldu!
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(market) 
    ...
    if(args[0] === "sil") {
        ...
        if(linkler.length === 1) {
            ...
            .setColor(client.ayarlar.renk)
            .setDescription(`
            <:neutralyes:910935020120735806> Başarılı bir şekilde belirttiğiniz barkodu sistemimden kaldırdım!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(yardım)
        } else {
