...
exports.run = async(client, message, args, prefix) => {
    ...
    if(args[0] === "transfer") {
        ...
        if(!db.fetch(`goldkredi_${message.author.id}`)) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Hiç Bir Gold Kredisine Sahip Değilsiniz!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(market) 
        }
        ...
        if(db.fetch(`goldkredi_${message.author.id}`) < miktar) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Gold Kredinızda **${miktar}** Miktarında kredi yok!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(market) 
        }
    ...
    if(args[0] === "bilgi") {
        ...
        if(kişi.id === client.user.id) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Benim kredi bilgilerime bakamazsın!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(market) 
        }
