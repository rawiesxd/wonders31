...
exports.run = (client, message, args, prefix) => {
    ...
    if(args[0] === "aç") {
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralyes:910935020120735806> Başarılı bir şekilde Pasif Mod **Aktifleştirildi!**
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(market)    
    }
    ...
    if(args[0] === "kapat") {
        if(!db.fetch(`pasifmod_${message.author.id}`) === "aktif") {
            ...
            .setDescription(`
            Hata!
            <:neutralno:910935142707646484> Pasif Mod zaten kapalı!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(market)
        }
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralyes:910935020120735806> Başarılı bir şekilde Pasif Mod **Kapatıldı!**
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(market)
    }
