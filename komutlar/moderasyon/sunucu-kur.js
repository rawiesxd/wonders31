 ...
 // const DBL = require("dblapi.js");
  //const dbl = new DBL(`eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6Ijc0NDUwNjQ5NjI1NTAwMDU5NiIsImJvdCI6dHJ1ZSwiaWF0IjoxNjA2MDQxMjA3fQ.vUs8drqgyaCvQOuih0CE11sqsa1n2ZwYiR_lkq8e4mo`,client)
    ...
    //dbl.hasVoted(message.author.id).then(voted => {
      //if(voted === true) {
        if(message.author.id !== message.guild.ownerID) return message.channel.send("<:neutralno:910935142707646484> Bu komutu kullanabilmek için sunucu sahibi olman gerekli!") 
		if(!message.guild.members.cache.get(client.user.id).hasPermission("ADMINISTRATOR")) return message.channel.send(`<:neutralno:910935142707646484> ${exports.help.name} Adlı komutu çalıştırabilmem için \`YÖNETİCİ\` yetkisine ihtiyacım var!`)
        if(!args[0]) {
          const embed = new Discord.MessageEmbed()
          .setAuthor(`${client.ayarlar.botİsim} Bot`, message.author.avatarURL({ dynamic: true }))
          .setColor(client.ayarlar.embedRenk)
          .setDescription(`
          <:neutralno:910935142707646484> Selam ${message.author}, Eğer sunucu kurmak istiyorsan bir değer belirt!
        ...
        if(args[0] === 'normal') {
           ...
           .setColor(client.ayarlar.embedRenk)
           .setDescription(`
           <:wsoru:911958772421918720> Selam sayın ${message.author}, Eğer **Normal Sunucu** Kurulumu yapmak istiyorsanız **evet** yazın, iptal edilmesini istiyorsanız 10 saniye bekleyin.
           `)
           .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
      ...
      if(args[0] === 'youtuber') {
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:wsoru:911958772421918720> Selam sayın ${message.author}, Eğer **YouTuber Sunucu** Kurulumu yapmak istiyorsanız **evet** yazın, iptal edilmesini istiyorsanız 10 saniye bekleyin.
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed).then(normal => {
         message.channel.awaitMessages(response => response.content === 'evet', {
      ...
      if(args[0] === 'kod') {
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:wsoru:911958772421918720> Selam sayın ${message.author}, Eğer **Kod Sunucu** Kurulumu yapmak istiyorsanız **evet** yazın, iptal edilmesini istiyorsanız 10 saniye bekleyin.
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed).then(normal => {
         message.channel.awaitMessages(response => response.content === 'evet', {
      ...
      if(args[0] === 'destek') {
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:wsoru:911958772421918720> Selam sayın ${message.author}, Eğer **Destek Sunucu** Kurulumu yapmak istiyorsanız **evet** yazın, iptal edilmesini istiyorsanız 10 saniye bekleyin.
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed).then(normal => {
         message.channel.awaitMessages(response => response.content === 'evet', {
