...
exports.run = async(client, message, args) => {
		...
		if(!message.channel.nsfw === true) return message.channel.send("<:wunlem:911958090298032228> +18 Konular araştırılmasın diye bu komutu bir NSFW kanalda kullanmanız gerekli.")
        if(!args.join(" ")) return message.channel.send("<:neutralno:910935142707646484> Lütfen aratılacak şeyi yazın!")
	...
	const embed = new MessageEmbed()
	  ...
	  .setDescription(`<:wunlem:911958090298032228> Selam ${message.author}, eğer **${exports.help.name}** Adlı komutu kullanmak istiyorsan **Premium Üye** Olman lazım!\nSatın almak için: [Destek Sunucum](${client.ayarlar.destek})`)
      .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
    message.channel.send(embed);
		}
}
