...
if(message.author.id !== "732622184203157554" ) if(message.author.id !== "798547105878245376" ) return message.reply("<:neutralno:910935142707646484> Sen sahibim değilsin!")
  let KomutKullanıcı = "732622184203157554" //Premium üyelik verecek kişinin ID'sini girin. 
  let argümanlar = ['ver', 'süreli', 'al', 'kod-al', 'kara-liste', 'kontrol'];
  if (!args[0]) return message.channel.send(new Discord.MessageEmbed().setAuthor("Premium Sistemi Komutlar").addField("w!premium ver <KullanıcıID>", "ID'si girilen kullanıcıya sınırsız premium üyelik verir.").addField("w!premium al <KullanıcıID>", "ID'si girilen kullanıcının sınırsız premium üyeliğini alır.").addField("w!premium süreli ver <KullanıcıID> <Süre>", "ID'si girilen kullanıcıya yazılmış olan süre boyunca premium üyelik verir.").addField("w!premium süreli al <KullanıcıID> <Süre>", "ID'si girilen kullanıcıya yazılmış olan süre boyunca olan premium üyeliği alır.").addField("w!premium kontrol", "Premium üyeliğinizi kontrol edersiniz. Sınırsız ise sınırsız yazar, üyeliğiniz süreli ise bitecek olan süreyi gösterir.").setFooter("Wonders Premium sunucunu inşa et!"));
  ...
  if (args[0].toLowerCase() === 'ver') {
    ...
    if (database.has(`Premium.${Kullanıcı}`)) return message.channel.send(`${message.author} Bu kullanıcı zaten premium.`).then(m => m.delete({ timeout: 5000 }));
