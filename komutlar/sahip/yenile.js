...
exports.run = async(client, message, args) => {
  if(message.author.id !== "732622184203157554" ) if(message.author.id !== "798547105878245376" ) return message.reply("<:neutralno:910935142707646484> Sen sahibim değilsin!")
    ...
    if(!args.length) return message.channel.send(`<:neutralno:910935142707646484> Lütfen yenilenecek komutu söyleyin.`);
  ...
  if(!command) return message.channel.send(`<:neutralno:910935142707646484> \`${args[1]}\` Adlı komut bulunamadı!`);
  ...
  message.channel.send(`<:neutralyes:910935020120735806> \`${command}\` Adlı komut yenilendi.`);
}
