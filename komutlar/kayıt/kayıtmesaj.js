...
exports.run = async(client, message, args, prefix) => {
    ...
    if(args[0] === "ayarla") {
        if(!kanal) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Lütfen bir mesaj belirtin!
            `)
        ...
        if(db.fetch(`kayıtmesaj_${message.guild.id}`)) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Kayıt Mesajı Zaten Ayarlanmış!
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralyes:910935020120735806> Başarılı bir şekilde **Kayıt Mesajını** ayarladım!
        `)
    ...
    if(args[0] === "sıfırla") {
        if(!db.fetch(`kayıtmesaj_${message.guild.id}`)) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Kayıt Mesajı Zaten Sıfırlanmış/Ayarlanmamış!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(embed)
        }
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralyes:910935020120735806> Başarılı bir şekilde **Kayıt Mesajını** sıfırladım!
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed)
    }
