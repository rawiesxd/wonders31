...
exports.run = async(client, message, args, prefix) => {
    ...
    if(args[0] === "ayarla") {
        ...
        if(db.fetch(`kayıtkızrol_${message.guild.id}`)) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Kayıt Erkek Rolü Zaten Ayarlanmış!
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralyes:910935020120735806> Başarılı bir şekilde **Kayıt Kız Rolünü** ${kanal} olarak ayarladım!
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed)
    }
    ...
    if(args[0] === "sıfırla") {
        if(!db.fetch(`kayıtkızrol_${message.guild.id}`)) {
            ...
            .setColor(client.ayarlar.embedRenk)
            .setDescription(`
            <:neutralno:910935142707646484> Kayıt Kız Rolü Zaten Sıfırlanmış/Ayarlanmamış!
            `)
            .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
            return message.channel.send(embed)
        }
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralyes:910935020120735806> Başarılı bir şekilde **Kayıt Kız Rolünü** sıfırladım!
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed)
    }
