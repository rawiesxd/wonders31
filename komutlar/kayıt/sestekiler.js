...
exports.run = async(client, message, args, prefix) => {
    if(message.author.id !== "732622184203157554" ) if(message.author.id !== "798547105878245376" ) return message.reply("<:neutralno:910935142707646484> Bu komut geçici olarak **kapalıdır!**")
    ...
    if(!message.guild.members.cache.filter(s => s.roles.cache.has(yetkili))) {
        ...
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`
        <:neutralno:910935142707646484> <@&${yetkili}> Rolüne ait birisi bulunamadı.
        `)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
        return message.channel.send(embed)
    }
